...
static iConState;                           // current connection state ( 0 = off, 1 = scanning,
                                            // 2 = connecting, 3 = active)
...
const
{
  ...
  //Measurement state
  MEASUREMENT_TEMP   = 0,                   // Read temperature value
  MEASUREMENT_rH     = 1,                   // Read humidity value
}
...
Handle_Led()
{
  if(iBLE_Enable == 0)Led_Off();            // if BLE is switched off -> Switch off LED
  else                                      // Otherwise ->
  {
    // if a BLE connection is established -> Switch on LED (blue)
...
Handle_BLE()
{
  ...
  iBLEState = BLE_GetState();               // Read current BLE status from the system
#if defined DEBUG
  printf("BLE State: %d\r\n", iBLEState);
#endif
  if(iBLEState == BLE_STATE_OFF)            // If the BLE interface is off ->
  {
    ...
    iConState = STATE_OFFLINE;              // Set BLE Connection state to "offline"
  ...
  else if(iBLEState == BLE_STATE_READY)     // Otherwise -> If the BLE interface is ready ->
  {
    if(iBLE_Enable == 0)                    // If BLE is switched off ->
    {
      ...
      {
        BLE_Disconnect();                   // Disconnect the device
        printf("disconntecting...\r\n");
      }
      else                                  // Otherwise ->
      {
        /* power off BLE */
        BLE_Close();                        // Close and deactivate BLE interface.
    ...
    else                                    // If BLE is active ->
    {
          ...
          (iConState == STATE_SCANNING)   )
      {
        ...
        if(BLE_Scan(10, 1) >= OK)           // If an "active scan" with 10sec. scan time could be started ->
        {
          ...
          iConState = STATE_SCANNING;       // Set BLE Connection state to "Scanning for BLE devices"
        }
      }
      ...
      else if (iConState == STATE_CONNECTING)
      {
        ...
        //If the BLE interface could be connected to the BLE device -> Set BLE Connection state to "BLE connection established"
      ...
      else if (iConState == STATE_ACTIVE)
      {
        ...
        switch(iMeasurementState)           // Switch current measurement state ->
        {
          case MEASUREMENT_TEMP:            // Temperature value should be read
          {
            iResult = BLE_Read(0,0x0032);   // Read temperatur value from the "Smart Humigadget" handle 0x32
            // If the read of the temperature could be triggered -> set current measurement state to "Read humidity value"
            if(iResult >= OK)iMeasurementState = MEASUREMENT_rH;
          }
          case MEASUREMENT_rH:              // humidity value should be read
          {
            iResult = BLE_Read(0,0x0037);   // Read humidity value from the "Smart Humigadget" handle 0x37
            // If the read of the humidity could be triggered -> set current measurement state to "Read temperature value"
            if(iResult >= OK)iMeasurementState = MEASUREMENT_TEMP;
          }
          //default -> set current measurement state to "Read temperature value"
...
public BLE_Event(event, connhandle, const data{}, len)
{
     ...
     (event == BLE_EVENT_SCAN_RSP))
  {
    ...
    {
      /* Choose this device to connect to by coping the temporary memory for the scan response to the
         static variable for the Scan response of the device to which a connection should be established*/
      sDevice = sScan;

      // Set BLE Connection state to "Connecting to a specific BLE device"
  ...
  else if(event == BLE_EVENT_READ)
  {
    ...
    switch(sRead.handle)                    // Switch the handle
    {
      case 0x0032:                          // Data contain the temperature value (handle 0x32)
      {
        if(sRead.data_len == 4)
        {
      ...
      case 0x0037:                          // Data contain the humidity value (handle 0x37)
      {
        if(sRead.data_len == 4)
        {
    ...
    {
      iFlags = 0;                           // Clear Flags indicating which values are valid
...
public KeyChanged(iKeyState)
{
  ...
  if(!iKeyState)                            // If the button has been released->
  {
    if(iBLE_Enable == 0)                    // If BLE is switched off ->
    {
      ...
      {
        iBLE_Enable = 1;                    // Activate BLE
        print("BLE ON\r\n");
      }
    }
    else                                    // Otherwise ->
    {
      iBLE_Enable = 0;                      // Switch off BLE
      print("BLE OFF\r\n");
    }
  }                                              
}

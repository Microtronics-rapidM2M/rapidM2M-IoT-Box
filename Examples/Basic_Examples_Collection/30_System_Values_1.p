 ...
 *
 * Reads the last valid values for VBat, VUsb and Temp periodically (record interval) from the system
 * and stores the generated data record in the flash of the system. The measurement data generated this
 * way is then transmitted periodically (transmission interval) to the server. 
 * 
...
const
{
  ...
  SIZE_RECDATA    = 1+4+4+4,                // Measurement data block consists of:
                                            // "Split-tag"(u8) + VBat(u32) + VUsb(u32) + Temp(s32)
}
...
/* Reads the last valid values for VBat, VUsb and Temp from the system, compiles the data
   record to be saved and transfers the compounded data record to the system to be recorded */
RecordData()
{
  ...
  /* Compiles the data record to be saved in the "aRecData" temporary memory
     - The first byte (position 0 in the "aRecData" array) is set to 0 so that the server copies
       the data record into measurement data channel 0 upon receipt, as specified during the design
         of the connector
     - VBat is copied to position 1-4.  Data type: u32
     - VUsb is copied to position 5-8.  Data type: u32
     - Temp is copied to position 9-12. Data type: s32                                       */
  aRecData{0} = 0;                          // "Split-tag" 
  rM2M_Pack(aRecData, 1, aSysValues.VBat, RM2M_PACK_U32|RM2M_PACK_BE);
  rM2M_Pack(aRecData, 5, aSysValues.VUsb, RM2M_PACK_U32|RM2M_PACK_BE);
  rM2M_Pack(aRecData, 9, aSysValues.Temp, RM2M_PACK_S32|RM2M_PACK_BE);

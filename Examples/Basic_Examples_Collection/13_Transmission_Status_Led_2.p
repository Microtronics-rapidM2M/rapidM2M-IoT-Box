 ...
 * LED is off 	                        if the device is offline (not connected)
 ...
 * LED blinks 1x green ever 3 sec. 	if the device is in "Wakeup" mode
 * LED blinks 2x red ever 3 sec. 	if the connection establishment has failed
 *
 *
...
const 
{
  ...
  STATE_OFF = 0,                          // Device is offline (not connected)                        (LED off)
  STATE_ON,                               // Device is connected to the server                        (LED on)
  STATE_SLOW,                             // Device is in "Wakeup" mode                               (LED blinking green 1x every 3 seconds)
  STATE_FAST,                             // Connection establishment has started or
                                          // is waiting for the next automatic retry                  (LED flickering)
  STATE_ERROR,                            // Connection establishment failed                          (LED blinking red 2x every 3 seconds)
}
  ...
  if(iTimerLed > 0)                         // If the counter used to control the LED has not expired ->
  {
    ...
    if(iTimerLed == 0)                      // If the counter used to control the LED has expired ->
    {
      switch(iState)                        // Switches current state that should be displayed via the LED 
      {
        ...
        case STATE_ON:
        {
          Led_On(0x00FF00);                 // Makes the LED light up green
        }
        case STATE_SLOW:
        {
          Led_Off();
          Led_Blink(1, 0x00FF00);           // Makes the LED blink 1x green
          iTimerLed = 3;                    // Sets the counter used to control the LED to "3" so that the "HandleLed" function is called up again after 3 seconds
        }
        case STATE_FAST:
        {
          Led_Off();
          Led_Flicker(0, 0x00FF00);         // Makes the LED flicker green until another Led_xxx function is called
        }
        case STATE_ERROR:
        {
          Led_Off();
          Led_Blink(2, 0xFF0000);           // Makes the LED blink 2x red
          iTimerLed = 3;                    // Sets the counter used to control the LED to "3" so that the "HandleLed" function is called up again in 3 secounds
        }
      }
    }
  }

 ...
 *
 * The state machine has four different states that are indicated by the RGB LED
 * - 0  LED flashes continuously red 
 * - 1  LED blinks green continuously
 * - 2  LED flashes 3x green every 5sec.
 * - 3  LED blinks 3x red every 5sec.
 *
...
main()
{
 /* Temporary memory for the index of a public function and the return value of a function  */
  ...
  Timer5s();                                // Calls the timer callback function to display the initial state of the application using the RGB LED
}
...
public Timer5s()
{
  ...
  if(iState == 0)
  {
    // LED should flash red continuously
    print("red flashing cont.\r\n");
    Led_Off();
    Led_Flash(0, 0xFF0000); 
  }
  else if(iState == 1)
  {
    // LED should blink green continuously
	print("green blinking cont.\r\n");
    Led_Off();
    Led_Blink(0, 0x00FF00);
  }
  else if(iState == 2)
  {
    // LED should flash 3x green every 5sec.
    print("3x flash green\r\n");
    Led_Off();
    Led_Flash(3, 0x00FF00);
  }
  else if(iState == 3)
  {

 ...
 *
 * Reads the last valid values for VBat, VUsb and Temp periodically (record interval) from the system and stores the 
 ...
 * measurement data as well as the transmission mode (interval, wakeup or online) can be configured via the server.
 * 
...
const
{
  ...
  SIZE_RECDATA    = 1+4+4+4,                // Measurement data block consists of: "Split-tag" (u8) + VBat (u32) + VUsb (u32) + Temp (s32)
...
/* Reads the last valid values for VBat, VUsb and Temp from the system, compiles the data record to  
